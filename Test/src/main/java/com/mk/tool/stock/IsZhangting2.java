 ...
 * 大底
 */
public class IsZhangting2 extends Stragety {
    public static Table table;
    public static double  minZF = 1.3f;
    ...
    public static Row getRow(String file, String INFO, String date, Kline kline0Prev, MinuteLine minuteLine, LineContext context) {
        ...
        String type = IsZhangting2.class.getSimpleName();
        String zfStr = "";
        if (Kline != null) {
            if (minuteLine != null) {
                MinuteLine lastMinuteLine = minuteLine.allLineList.get(minuteLine.allLineList.size() - 1);
        ...
        com.mk.tool.stock.Kline.SortEntity[] aobjs = KlinePrev.getDZNum(0);
...
//        w30d60	w30d120	w30d250	w60d120	w60d250	w120d250
        ...
        row.add(new Col(StringUtil.format((Float) jumps[0])));
        row.add(new Col("" + jumps[1]));
        row.add(new Col("" + jumps[5]));
        row.add(new Col("" + jumps[2]));
        row.add(new Col("" + StringUtil.format((Float) jumps[3])));

        row.add(new Col(StringUtil.format((Float) jumps2[0])));
        ...
        row.add(new Col("" + StringUtil.format((Float) horFlag[2])));

        row.add(new Col("" + horFlag2[0]));
        row.add(new Col("" + horFlag2[1]));
        row.add(new Col("" + StringUtil.format((Float) horFlag2[2])));
...
//        row.add(new Col(StringUtil.formatVs2(listMASpaceWeek)));
        ...
        com.mk.tool.stock.Kline.LocalBottom localBottom = KlinePrev.getLocalBottom();
        row.add(new Col("" + localBottom.flag));
        row.add(new Col("" + localBottom.num));
        row.add(new Col(StringUtil.format(localBottom.frac)));

        com.mk.tool.stock.Kline.LocalBottom localWBottom1 = KlinePrev.getWLocalBottomW(2.6f, 10);
        row.add(new Col("" + localWBottom1.flag));
        row.add(new Col("" + localWBottom1.num));
        row.add(new Col(StringUtil.format(localWBottom1.frac)));

        com.mk.tool.stock.Kline.LocalBottom localWBottom = KlinePrev.getWLocalBottomWBZ(3f, 10);
        ...
        LineReport lineReport = IsBottom.report(file, KlinePrev, context, context.isRealBottomFlag());
        if (lineReport != null) {
        ...
        row.add(new Col("" + filterRowSpecialHorAfter(kline0Prev)));
        if (minuteLine != null) {
            double cur = KLineUtil.compareMaxSign(minuteLine.price, open);
            row.add(new Col("" + cur));
        } else {
...
//        row.add(new Col(yueyali));
    ...
    public static int filterRowSpecialHorAfter(Kline kline0) {
        ...
        boolean horFrac = kline0.allIsLessAbsThanExcept(5, 0, 1.0f, 0, 0.0f, 0, 0.0f, 1.5f);
        if (horFrac) {
            return 1;
        }
    ...
    public static void ok(String file, String INFO, String date, Kline kline0, MinuteLine minuteLine, Weekline weekline, Kline nextN, String msg, LineContext context) {
        ...
        int lft = (int) aRow.getInt("LFT");
        int rgt = (int) aRow.getInt("RGT");
        double open = kline0.prev().getOpen();
        double mtzf = IsZhangting2.getMtZF(open, minuteLine);
        boolean ret = false;
        if (mtzf <= 1.835) {
            if (lft == 1) {
                ret = true;
...
//        }else {
            if (ret) {
//                boolean flag = IsBottom.filterRowAfter(aRow.getTable(), aRow);
//                if (!flag) {
//                    return;
//                }
                if (mtzf > minZF) {
                    Row row = IsZhangting2.getRowZT(file, INFO, date, kline0.prev(), minuteLine, context);
                    table.add(row);
                    Log.logString(StragetyBottom.resultBuffer, IsBottom.getINFO(INFO) + " " + date + " " + minuteLine.getTime());
                }
            }
...
//                    } catch (Exception e) {
//                        com.mk.util.Log.log("ERR:" + context.getInfo());
//                    }
//                }
//            }
//        }
...
//                result.stragety = IsZhangting.class.getSimpleName();
...
//        Log.log("prs==========>Y " + INFO + "	" + date + "   offsetLen:");
    }
