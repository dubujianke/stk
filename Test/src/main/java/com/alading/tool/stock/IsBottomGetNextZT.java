...
import com.alading.tool.stock.tool.SelectExcel;
 ...
 * 大底
 */
public class IsBottomGetNextZT extends Stragety {
    ...
    public static void ok(String file, String INFO, String date, Kline kline0, Weekline weekline, Kline nextN, String msg, LineContext context) {
        Table table = IsBottom.getTable(file, INFO, date, kline0, null, context);
        table.initIndex();
        boolean flag = SelectExcel.isRealOK(kline0, table,  table.rows.get(1));
        context.setType05(1);
        if (flag) {
...
//                e.printStackTrace();
    ...
    public void prsIsN(String file, List<Kline> days, String date, List<Weekline> weeks, List<MonthKline> moths, int usemonth, int useweek, LineContext context) {
        if (file.contains("002340") && date.equalsIgnoreCase("2024/03/14")) {
            int a = 0;
        }
        ...
        double guben = GetGuben.retriveOrGetShizhi(AbsStragety.getCode(file), kline0.getClose());
        if(guben>100) {
            return;
        }
        if(guben>300) {
        ...
        if(kline0.getZhangfu()>5.0) {
            return;
        }
        if(kline0.prev().getZhangfu()>5) {
            return;
        }
