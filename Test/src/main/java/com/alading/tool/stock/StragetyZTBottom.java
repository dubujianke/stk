...
public class StragetyZTBottom extends AbsStragety {
    ...
    public static boolean isTHS = false;

    public static String getTDX(String timeTHS) {
        if (isTHS) {
            return timeTHS;
        }
        Date date = DateUtil.strToTime("2024-01-01 " + timeTHS+":00");
        date = DateUtil.getBeforeMinute(date, 1);
        return DateUtil.dateToString4(date);
    }

    public static int getDltTDX(int dlt) {
        if (isTHS) {
            return dlt;
        }
        return dlt-1;
