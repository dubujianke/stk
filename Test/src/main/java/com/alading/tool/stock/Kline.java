...
public class Kline {
    ...
    public double getMaxZhangfu(int num) {
        Kline day0 = allLineList.get(getIdx());
...
//                System.out.println(i+" "+item.getDate()+" "+item.getClose());
    ...
    public double getNextZTSpaceMAI(int period) {
        double v1 = KLineUtil.compareMaxSign(getClose() * 1.1, getMAI(period));
        return v1;
    }

    public boolean isNextZTSpace12(double min, double max) {
        double v1 = KLineUtil.compareMaxSign(getClose() * 1.1, getMA120());
        if (isBetween(v1 - 10, min, max)) {
            return true;
        }
        return false;
    }
    ...
    public boolean isNextZTSpace6(double min, double max) {
        double v1 = KLineUtil.compareMaxSign(getClose() * 1.1, getMA60());
        if (isBetween(v1 - 10, min, max)) {
            return true;
        }
        return false;
    ...
    public boolean isNextZTSpace3(double min, double max) {
        double v1 = KLineUtil.compareMaxSign(getClose() * 1.1, getMA30());
        if (isBetween(v1 - 10, min, max)) {
            return true;
        }
        return false;
    ...
    public boolean touch(double v) {
        double max = Math.max(this.getMin(), this.getMax());
        double min = Math.min(this.getMin(), this.getMax());
        double afrac = KLineUtil.compareMax(max, v);
        if (afrac < 1) {
            return false;
        }
        if (max >= v && min <= v) {
            double frac = KLineUtil.compareMax(max, v);
            if (frac < 1) {
                return false;
            }
            return true;
...
//    }
    ...
    public boolean szWuli() {
        ...
        if (isShadownUp()) {
            double fracv = 100.0 * getVolume() / prev.getVolume();
            if (fracv > 9.0f) {
                return true;
            }
        }
...
//        }
    ...
    public boolean isGoldOrComingGold(int ma1, int ma2) {
        ...
        double v1ma1 = this.getNextSupposeMAI(close * 1.1f, ma1);
        double v1ma2 = this.getNextSupposeMAI(close * 1.1f, ma2);
    ...
    public boolean isStandOnMA60() {
        double ma0 = getMAI(60);
        double v60 = KLineUtil.compareMaxSign(getOpen(), ma0);
        if (isBetween(v60, 0, 1)) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public boolean isStandMA30() {
        double ma30 = getMAI(30);
        double v30 = KLineUtil.compareMaxSign(getOpen(), ma30);
        double v30Prev = KLineUtil.compareMaxSign(prev().getOpen(), ma30);
        if (isBetween(v30, 0, 1) && isBetween(v30Prev, -3, 0)) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public boolean isStandMA10() {
        double ma10 = getMAI(10);
        double v10 = KLineUtil.compareMaxSign(getOpen(), ma10);
        if (isBetween(v10, 0, 1.8)) {
            return true;
        }
        return false;
    }
...
//    }
    ...
    public double getSlant(int period, int n) {
        ...
        double angle = 3 * (180 * Math.atan(slant) / Math.PI);
        return angle;
    }
    ...
    public double getABSSlant(int period, int n) {
        ...
        double angle = (180 * Math.atan(slant) / Math.PI);
        return angle;
    }
    ...
    public Object[] getJumpIndays(int num, boolean isSZ) {
        ...
        return new Object[]{0.0, false, -1, 0.0, 0, false};
    }
    ...
    public LocalBottom getNullLocalBottom() {
        com.alading.tool.stock.Kline.LocalBottom localBottom = new LocalBottom();
        return localBottom;
    }

    public LocalBottom getLocalBottom() {
        com.alading.tool.stock.Kline.LocalBottom localBottom = getNullLocalBottom();
        try {
            if (!localBottom.flag) {
    ...
    public LocalBottom getLocalBottomW(double fracV, int days) {
        ...
        com.alading.tool.stock.Kline.LocalBottom localBottom = klineMin.getLocalBottom(fracV, 43, 11, 7);
        return localBottom;
    }

    public LocalBottom getLocalBottomWBZ(double fracV, int num) {
        Kline klineMin = getPrevMINKlineSelf(num);
        com.alading.tool.stock.Kline.LocalBottom localBottom = klineMin.getLocalBottomBZ(fracV, 33, 11, 7);
        return localBottom;
    }
    ...
    public LocalBottom getWLocalBottomW(double fracV, int num) {
        Kline klineMin = getPrevMINKlineSelf(num);
        com.alading.tool.stock.Kline.LocalBottom localBottom = klineMin.getLocalBottom(fracV, 50, 30, 30);
        return localBottom;
    }
    ...
    public LocalBottom getWLocalBottomWBZ(double fracV, int num) {
        ...
        com.alading.tool.stock.Kline.LocalBottom localBottom = klineMin.getLocalBottom2(fracV, 50, 15, 15);
        return localBottom;
    }
    ...
    public SortEntity[] getDZNum(int offset) {
        ...
        vector.sort(new Comparator<SortEntity>() {
            ...
            public int compare(SortEntity o1, SortEntity o2) {
                int ret = (int) ((int) 100.0 * (o2.v - o1.v));
                return ret;
            }
        });
    ...
    public boolean isWillDeadCross(int period1, int period2) {
        int num = 0;
        double vperiod1 = getMAI(period1);
        double vperiod2 = getMAI(period2);
        double curFrac = KLineUtil.compareMaxSign(vperiod1, vperiod2);
        //即将死叉，但是距离太大
        if (vperiod2 < vperiod1 && curFrac > 3) {
            return false;
        }
        for (int i = 0; i < 4; i++) {
            double v1 = prev(i).getMAI(period1);
            double v2 = prev(i).getMAI(period2);
            double curFraci = KLineUtil.compareMaxSign(v1, v2);
            if (curFrac < 0 && curFraci > 0) {
                return true;
            }
            if (curFrac < 0.8 && curFraci > 1.8) {
                return true;
            }
        }
