...
import static com.alading.tool.stock.StragetyZTBottom.MIN_3MINUTES_LARGEHAND_ZF;
...
//        }
    ...
    public MinuteLine getFirstSpeedUpNoVOL(Kline kline, int fracVolNum1, int fracVolNum2, int fracVolNum3, LineContext context) {
        int specialHor = context.getInt("specialHor");
        MinuteResult minuteResult = new MinuteResult();
        MinuteLine cur = this;
        if (idx < 3) {
...
//        }
    ...
    public int getDltIdxTHS() {
        String time1 = time.substring(0,2)+":"+time.substring(2);
        Date date = DateUtil.strToTime("2024-01-01 " + time1+":00");
        date = DateUtil.getAfterMinutes(date, 1);
        String v1 = DateUtil.dateToString4(date);
        if( v1.startsWith("0")) {
            v1 = v1.substring(1);
        }
        int v2 =  Integer.parseInt(v1);
        if(StragetyZTBottom.isTDX) {
            return v2;
        }
        return v2+1;
    }
