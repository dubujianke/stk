...
public class IsZhangting extends Stragety {
    ...
    public static Row getRowZT(String file, String INFO, String date, Kline kline0Prev, MinuteLine minuteLine, LineContext context) {
        ...
        double prev_3 = KlinePrev.prev(3).getEntityZhangfu();
        double prev_2 = KlinePrev.prev(2).getEntityZhangfu();
        double prev_1 = KlinePrev.prev(1).getEntityZhangfu();
        double cur_ = KlinePrev.prev(0).getEntityZhangfu();

        row.setCol(table.getColumn("prev_(3)"), StringUtil.format(prev_3));
        row.setCol(table.getColumn("prev_(2)"), StringUtil.format(prev_2));
        row.setCol(table.getColumn("prev_(1)"), StringUtil.format(prev_1));
        row.setCol(table.getColumn("cur_"), StringUtil.format(cur_));
        double min_entity = KLineUtil.getMinAbs(cur_, prev_1, prev_2, prev_3);
        row.setCol(table.getColumn("min_entity"), StringUtil.format(min_entity));
    ...
    public static Row getRow(String file, String INFO, String date, Kline kline0Prev, MinuteLine minuteLine, LineContext context) {
        ...
        String type = IsZhangting.class.getSimpleName();
        String zfStr = "";
        if (Kline != null) {
            if (minuteLine != null) {
                MinuteLine lastMinuteLine = minuteLine.allLineList.get(minuteLine.allLineList.size() - 1);
...
//        w30d60	w30d120	w30d250	w60d120	w60d250	w120d250
        ...
        row.add(new Col(StringUtil.format(KlinePrev.prev(3).getEntityZhangfu())));
        row.add(new Col(StringUtil.format(KlinePrev.prev(2).getEntityZhangfu())));
        row.add(new Col(StringUtil.format(KlinePrev.prev(1).getEntityZhangfu())));
        row.add(new Col(StringUtil.format(KlinePrev.prev(0).getEntityZhangfu())));
        ...
        double cur_ = KlinePrev.prev(0).getEntityZhangfu();
        double min_entity = KLineUtil.getMinAbs(cur_, prev_1, prev_2, prev_3);
        row.add(new Col(StringUtil.format(min_entity)));
